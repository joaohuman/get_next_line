...
/*   Updated: 2021/07/09 17:49:02 by jvictor-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*make_line_and_buf_of_read(char **line, char *buf_of_read, int bytes)
{
	...
	**line = ft_substr(buf_of_read, 0, position_of_brkline - 1);
	...
	if (bytes)
	{
		len_of_buf_of_read = ft_strlen(buf_of_read + position_of_brkline + 1);
		new_buf_of_read = ft_substr(buf_of_read, position_of_brkline + 1,
				len_of_buf_of_read);
		if (!new_buf_of_read)
			return (NULL);
	}
