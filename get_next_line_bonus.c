...
/*   Updated: 2021/07/11 13:15:36 by jvictor-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_next_line(int fd, char **line)
{
	...
	if (!buf_of_line)
		return (GNL_ERROR);
	verification = read_and_join(fd, &buf_of_read[fd], &buf_of_line, &bytes);
	if (verification == GNL_ERROR)
		return (GNL_ERROR);
	buf_of_read[fd] = make_line_and_buf_of_read(line, buf_of_read[fd], bytes);
	if (!bytes)
